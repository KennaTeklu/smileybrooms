...
// Define task counts for each tier (consistent with pricing-config)
...
export function EnhancedRoomCustomizationPanel({
  ...
  const getTierIcon = (tierName: string) => {
    ...
    if (tierName.includes("ELITE")) return <Zap className="h-4 w-4" />
    return <Shield className="h-4 w-4" />
  }

  const getTierColorClass = (tierName: string) => {
    if (tierName.includes("STANDARD")) return "text-blue-600"
    if (tierName.includes("PREMIUM")) return "text-purple-600"
    if (tierName.includes("ELITE")) return "text-green-600"
        ...
        <Tabs defaultValue="tiers" className="w-full">
          ...
          <TabsContent value="tiers" className="space-y-4">
            <RadioGroup value={localConfig.selectedTier} onValueChange={handleTierChange}>
              {tiers.map((tier) => {
                const currentTierTaskCount = TIER_TASK_COUNTS[tier.name.toUpperCase() as keyof typeof TIER_TASK_COUNTS]
                const eliteTierTaskCount = TIER_TASK_COUNTS.ELITE
                const progressValue = (currentTierTaskCount / eliteTierTaskCount) * 100
                          ...
                          <Label htmlFor={tier.name} className="cursor-pointer">
                            ...
                            {tier.detailedTasks && tier.detailedTasks.length > 0 && (
                              <div className="mt-3 text-sm">
                                ...
                                {tier.name !== BUNDLE_NAMING.ELITE && (
                                  <div className="mt-2">
                                    <Label className="text-xs text-gray-600 dark:text-gray-400">
                                      Progress towards Elite: {Math.round(progressValue)}%
                                    </Label>
                                    <Progress
                                      value={progressValue}
                                      className="w-full mt-1"
                                      indicatorColor={getTierColorClass(tier.name).replace("text-", "bg-")}
                                    />
                                  </div>
                                )}
