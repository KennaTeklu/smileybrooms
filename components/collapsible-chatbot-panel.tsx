...
// Define fixed top offsets for different states of the expanded panel
const EXPANDED_CHATBOT_TOP_OFFSET = 0 // When chatbot is expanded and no other panel is active
const SHARE_PANEL_ACTIVE_CHATBOT_TOP_OFFSET = 500 // When share panel is expanded (overrides other states for chatbot position)
...
const bottomPageMargin = 50 // Margin from the very bottom of the document for expanded panel
const minTopOffset = 0 // Minimum distance from the top of the viewport for expanded panel
...
export function CollapsibleChatbotPanel({
  ...
  // Calculate top position for expanded panel
  const expandedPanelTop = (() => {
    const documentHeight = document.documentElement.scrollHeight
    const desiredTop = sharePanelInfo.expanded ? SHARE_PANEL_ACTIVE_CHATBOT_TOP_OFFSET : EXPANDED_CHATBOT_TOP_OFFSET
    const currentPanelHeight = EXPANDED_PANEL_HEIGHT
    const maxPanelTop = documentHeight - currentPanelHeight - bottomPageMargin
    return `${Math.max(minTopOffset, Math.min(desiredTop, maxPanelTop))}px`
  })()

  const topTransition = sharePanelInfo.expanded ? "none" : "top 0.3s ease-in-out"
  ...
  return (
    <AnimatePresence initial={false}>
      {isExpanded ? (
        <motion.div
          key="expanded"
          ref={expandedPanelRef} // Ref for expanded panel
          initial={{ width: 0, opacity: 0, x: 20 }}
          animate={{ width: "auto", opacity: 1, x: 0 }}
          exit={{ width: 0, opacity: 0, x: 20 }}
          transition={{ type: "spring", damping: 25, stiffness: 300 }}
          className="fixed right-0 w-full sm:max-w-sm md:max-w-md lg:max-w-lg bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-l-2xl shadow-2xl overflow-hidden border-l-2 border-t-2 border-b-2 border-blue-200/50 dark:border-blue-800/50"
          style={{
        ...
        <motion.div
          ...
          exit={{ opacity: 0, x: 20 }}
          transition={{ type: "spring", damping: 25, stiffness: 300 }}
          className="fixed bottom-4 right-4" // Position for collapsed state
        >
          <Button
            ref={collapsedButtonRef} // Ref for collapsed button
            variant="outline"
            size="icon"
            className={cn(
