          ...
          <div key={item.id} className="flex items-center space-x-4 py-2 border-b">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted-foreground">
                {formatCurrency(item.price)} {item.paymentFrequency && `(${item.paymentFrequency.replace("_", " ")})`}
              </p>
              {item.metadata && item.metadata.rooms && (
                <p className="text-xs text-muted-foreground truncate">{item.metadata.rooms}</p>
              )}
            </div>
