...
import { roomImages, roomDisplayNames, defaultTiers, getPriceForTier } from "@/lib/room-tiers"
...
export function CollapsibleAddAllPanel({ isOpen, onOpenChange }: CollapsibleAddAllPanelProps) {
  ...
  // Auto-initialize all rooms with quantity 1 when modal opens
  React.useEffect(() => {
    if (isOpen) {
      Object.keys(roomDisplayNames).forEach((roomType) => {
        if (!roomCounts[roomType]) {
          updateRoomCount(roomType, 1)
          if (!roomConfigs[roomType]) {
            updateRoomConfig(roomType, {
              selectedTier: defaultTiers.default[0]?.id || "default-essential",
              totalPrice: getPriceForTier(defaultTiers.default[0]?.id || "default-essential"),
              selectedAddOns: [],
              selectedReductions: [],
              detailedTasks: defaultTiers.default[0]?.features || [],
              notIncludedTasks: [],
              upsellMessage: "",
            })
          }
