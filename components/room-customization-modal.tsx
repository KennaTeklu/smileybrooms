...
export function RoomCustomizationModal({
  ...
  const isDesktop = useMediaQuery("(min-width: 768px)") // Define desktop breakpoint
  ...
  const modalTitleId = `modal-title-${roomType}`
  const modalDescId = `modal-desc-${roomType}`

  const CommonContent = (
    <>
      <DialogHeader>
        {" "}
        {/* Use DialogHeader for both, it's just a div */}
        <DialogTitle id={modalTitleId} className="flex items-center gap-2 text-xl">
          <span className="text-2xl" aria-hidden="true">
            {roomIcon}
          </span>
          {roomName} Configuration
        </DialogTitle>
        <DialogDescription id={modalDescId}>Customize your {roomName.toLowerCase()} cleaning options</DialogDescription>
      </DialogHeader>
                      ...
                      {matrixServices.add.map((service, index) => (
                        <div key={index} className="flex items-start space-x-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => handleMatrixAddServiceChange(service.id, checked as boolean)}
                            className="mt-1"
                          />
                          <div className="grid gap-1.5 leading-none">
                            <Label
  ...
  return (
    <Drawer
      ...
      aria-describedby={modalDescId}
    >
      <DrawerContent className="max-h-[90vh]">{CommonContent}</DrawerContent>
    </Drawer>
  )
}
