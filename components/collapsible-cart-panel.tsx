...
import { ShoppingBag, Trash2, CheckCircle, AlertCircle, XCircle, Lightbulb, Tag, Minus, Plus } from "lucide-react"
...
export default function CollapsibleCartPanel() {
  ...
  const [isCartOpen, setIsCartOpen] = useState(true) // State to manage collapsible panel
    ...
    <div className="container mx-auto py-8 px-4 sm:px-6 lg:px-8 min-h-[calc(100vh-64px)] flex flex-col">
      ...
      {cart.items.length === 0 ? (
        ...
        <div className="grid lg:grid-cols-3 gap-8 flex-1">
          {/* Cart Items List */}
          <Collapsible open={isCartOpen} onOpenChange={setIsCartOpen} className="lg:col-span-2">
            <Card className="shadow-lg border-gray-200 dark:border-gray-700" id="cart-items-list">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <CollapsibleTrigger asChild>
                    <Button variant="ghost" size="icon" className="rounded-full">
                      {isCartOpen ? <Minus className="h-4 w-4" /> : <Plus className="h-4 w-4" />}
                      <span className="sr-only">{isCartOpen ? "Collapse" : "Expand"} cart items</span>
                    </Button>
                  </CollapsibleTrigger>
                </div>
              </CardHeader>
