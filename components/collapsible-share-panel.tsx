...
interface CollapsibleSharePanelProps {
  onPanelStateChange?: (info: { expanded: boolean; height: number; top: number }) => void
}
...
export function CollapsibleSharePanel({ onPanelStateChange = () => {} }: CollapsibleSharePanelProps) {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      if (panelRef.current && !panelRef.current.contains(event.target as Node) && isExpanded) {
        setIsExpanded(false)
        // Pass the current state including top position
        const newState = {
          expanded: false,
  ...
  }, [isExpanded, isMounted, onPanelStateChange]) // Removed panelTopPosition from dependencies
  ...
  const topPosition = isScrollPaused
  ...
  return (
    <div ref={panelRef} className="fixed right-0 z-[998] flex" style={{ top: topPosition }}>
